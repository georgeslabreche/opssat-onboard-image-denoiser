...
from noiser import generate_fixed_pattern_noise, add_noise_to_image
...
def load_and_preprocess_image(path, resize=False, apply_noise=False, noise_type=None, noise_factor=None):
  ''' load and preprocess image 
    ...
    @param noise_type: the noise type, 1 for FPN and 2 for cFPN
  ...
  if apply_noise is True:
    ...
    # pattern size for the FPN noise
    if noise_type in [1, 2]:
      ...
      noise_pattern = generate_fixed_pattern_noise(pattern_size, noise_factor)
